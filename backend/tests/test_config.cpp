...
TEST(ConfigTest, DefaultValues) {
    ...
    EXPECT_DOUBLE_EQ(cfg.hal.contrastResistance, 45.0);
    EXPECT_DOUBLE_EQ(cfg.hal.salineResistance, 35.0);
    EXPECT_DOUBLE_EQ(cfg.hal.baselinePressure, 10.0);
    EXPECT_DOUBLE_EQ(cfg.hal.motorTimeConstantMs, 50.0);
    EXPECT_DOUBLE_EQ(cfg.hal.maxRpm, 1500.0);
...
TEST(ConfigTest, ParseFullJson) {
    Config cfg = Config::loadFromString(R"({
        ...
        "hal": { "contrastResistance": 75.0, "salineResistance": 40.0, "baselinePressure": 15.0, "flowPerRpm": 0.02, "motorTimeConstantMs": 100.0, "maxRpm": 2000.0 },
